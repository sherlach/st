...
static int isvbellcell(int, int);
static void vbellbegin(void);

static void xfillcircle(int, int, int, uint);
static void xdrawvbell(void);
...
static char *titlestack[TITLE_STACK_SIZE]; /* title stack */
...
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len)
{
  ...
  xdrawline(line, 0, oy, len);
	...
	if (IS_SET(MODE_REVERSE)) {
      ...
      /* dynamic cursor color patch pt I*/
      g.bg = g.fg;
			g.fg = defaultbg;
		}
	}
  /* dynamic cursor color patch pt II
   * it handles the `drawcol` variable */
  if (IS_TRUECOL(g.bg)) {
    colbg.alpha = 0xffff;
    colbg.red = TRUERED(g.bg);
    colbg.green = TRUEGREEN(g.bg);
    colbg.blue = TRUEBLUE(g.bg); //true blue, mate
...
xsettitle(char *p, int pop)
{
  ...
  if (pop) {
    ...
    tstki = (tstki-1+TITLE_STACK_SIZE) % TITLE_STACK_SIZE;
...
xpushtitle(void)
{
  int tstkin = (tstki+1) % TITLE_STACK_SIZE;
...
isvbellcell(int x, int y)
{
  ...
  return vbellmode == 0 && (VBCELL);  /* logic defined in config.h */
}
